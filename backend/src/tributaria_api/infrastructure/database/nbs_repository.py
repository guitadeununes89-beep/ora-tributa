...
from tributaria_api.infrastructure.database.nbs_models import (
    NbsCatalogLifecycleEventRecord,
    NbsCatalogRecord,
...
class SqlAlchemyNbsRepository:
    def __init__(self, session: Session) -> None:
        self.session = session

    def get_version(self, organization_id: str, version_id: str) -> NbsCatalogVersionRecord:
        record = self.session.scalar(
            select(NbsCatalogVersionRecord).where(
                NbsCatalogVersionRecord.id == version_id,
                NbsCatalogVersionRecord.organization_id == organization_id,
            )
        )
        if record is None:
            raise NotFoundError("NBS catalog version not found")
        return record
    ...
    def ingest(
        ...
        *,
        ...
        catalog = self.session.scalar(
            select(NbsCatalogRecord).where(
                NbsCatalogRecord.organization_id == organization_id,
                NbsCatalogRecord.code == CATALOG_CODE,
            )
        )
        if catalog is None:
            catalog = NbsCatalogRecord(
                id=str(uuid4()),
                organization_id=organization_id,
                code=CATALOG_CODE,
                name="Nomenclatura Brasileira de Serviços (NBS)",
        ...
        existing_artifact = self.session.scalar(
            select(NbsCatalogVersionRecord).where(
                NbsCatalogVersionRecord.catalog_id == catalog.id,
                NbsCatalogVersionRecord.artifact_hash == parsed.artifact_hash,
            )
        )
        ...
        existing_version = self.session.scalar(
            select(NbsCatalogVersionRecord).where(
                NbsCatalogVersionRecord.catalog_id == catalog.id,
                NbsCatalogVersionRecord.version == metadata["version"],
            )
        )
        if existing_version is not None:
            raise ConflictError("NBS catalog version already exists with a different artifact")
        ...
        version = NbsCatalogVersionRecord(
            id=str(uuid4()),
    ...
    def list_versions(
        ...
        statement = select(NbsCatalogVersionRecord).where(
            NbsCatalogVersionRecord.organization_id == organization_id
        )
        if status:
            statement = statement.where(NbsCatalogVersionRecord.status == status)
        elif not include_unpublished:
            statement = statement.where(NbsCatalogVersionRecord.status == CatalogStatus.PUBLISHED)
        return [
            _version_view(item)
            for item in self.session.scalars(
                statement.order_by(NbsCatalogVersionRecord.publication_date.desc())
            )
        ]
    ...
    def search(
        ...
        statement = select(NbsCodeRecord).where(
            NbsCodeRecord.catalog_version_id == selected_version.id
        )
        if q:
            term = f"%{q.casefold()}%"
            statement = statement.where(
                or_(
                    func.lower(NbsCodeRecord.code).like(term),
                    func.lower(NbsCodeRecord.description).like(term),
                )
            )
        return [
            _code_view(row, selected_version)
            for row in self.session.scalars(statement.order_by(NbsCodeRecord.code))
        ]
    ...
    def _published_version(
        ...
        statement = select(NbsCatalogVersionRecord).where(
            NbsCatalogVersionRecord.organization_id == organization_id,
            NbsCatalogVersionRecord.status == CatalogStatus.PUBLISHED,
        )
        if version:
            statement = statement.where(NbsCatalogVersionRecord.version == version)
        record = self.session.scalar(
            statement.order_by(NbsCatalogVersionRecord.publication_date.desc()).limit(1)
        )
        if record is None:
            raise NotFoundError("Published NBS catalog version not found")
        return record


def _source(version: NbsCatalogVersionRecord) -> dict[str, Any]:
    return {
...
def _version_view(version: NbsCatalogVersionRecord) -> dict[str, Any]:
    return {
        "id": version.id,
...
def _code_view(row: NbsCodeRecord, version: NbsCatalogVersionRecord) -> dict[str, Any]:
    return {
        "code": row.code,
        "level": row.level,
